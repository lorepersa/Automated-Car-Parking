		...
		return { //this:ActionBasciFsm
				...
				state("carEnterIndoorArea") { //this:State
					action { //it:State
					 ...
					 transition(edgeName="t08",targetState="receiveSlotnum",cond=whenReply("rep_client_accept_in"))
				}	 
				state("receiveSlotnum") { //this:State
					action { //it:State
						...
						stateTimer = TimerActor("timer_receiveSlotnum", 
							scope, context!!, "local_tout_mastermind_receiveSlotnum", 1000.toLong() )
					}
					 transition(edgeName="t09",targetState="carDetectedByWeightSensor",cond=whenTimeout("local_tout_mastermind_receiveSlotnum"))   
				}	 
				state("carDetectedByWeightSensor") { //this:State
					action { //it:State
						println("[mastermind] | [State] carDetectedByWeightSensor | Entry point.")
						 val W = 800  
						forward("input_weight", "input_weight($W)" ,"weightsensor" ) 
					 ...
					 transition(edgeName="t010",targetState="pressCarEnterButton",cond=whenTimeout("local_tout_mastermind_carDetectedByWeightSensor"))   
				}	 
				state("pressCarEnterButton") { //this:State
					action { //it:State
					 ...
					 transition(edgeName="t011",targetState="readTokenId",cond=whenReply("rep_client_car_enter"))
				}	 
				...
				state("pickupRequest") { //this:State
					action { //it:State
					 ...
					 transition(edgeName="t012",targetState="carDetectedByOutSonar",cond=whenReply("rep_client_accept_out_success"))
				}	 
				state("carDetectedByOutSonar") { //this:State
					action { //it:State
						println("[mastermind] | [State] carDetectedByOutSonar | Entry point.")
						 val  D = 10  
						forward("input_distance", "input_distance($D)" ,"outsonar" ) 
						println("[mastermind] | [State] carDetectedByOutSonar | Exit point.")
						stateTimer = TimerActor("timer_carDetectedByOutSonar", 
							scope, context!!, "local_tout_mastermind_carDetectedByOutSonar", 7000.toLong() )
					}
					 transition(edgeName="t013",targetState="carLeaveOutdoorArea",cond=whenTimeout("local_tout_mastermind_carDetectedByOutSonar"))   
				}	 
				state("carLeaveOutdoorArea") { //this:State
					action { //it:State
					 ...
					 transition(edgeName="t014",targetState="end",cond=whenTimeout("local_tout_mastermind_carLeaveOutdoorArea"))   
				}	 
