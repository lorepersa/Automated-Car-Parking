...
import itunibo.automatedcarparking.dsl.util.IQActorRef
import itunibo.automatedcarparking.dsl.util.QActorRef

object SingletonTransportTrolleyQActor : ITransportTrolleyQActor {
	
	val sem = Semaphore(1)
	lateinit var transporttrolley : IQActorRef 
	val senderName = "internaltransporttrolley"
	
	init {
		try {
			transporttrolley = QActorRef("transporttrolley")
		} catch (e : Exception) {
	...
	override fun connect(host : String, port : Int) {
		transporttrolley = TcpQActorRef("transporttrolley", host, port)
	}
